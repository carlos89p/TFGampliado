...
from .injury_rules import (
    adapt_volume_for_injuries,
    build_injury_progression_notes,
    enrich_restrictions_for_injuries,
    has_low_back_issue,
)
...
def build_day(
    ...
    volume = adapt_volume_for_injuries(
        volume=volume,
    ...
    if has_low_back_issue(user_profile, restrictions) and "abdomen" not in muscle_slots:
        muscle_slots[-1] = "abdomen"
...
def build_routine(user_data: dict, catalog: list[dict]) -> dict:
    ...
    restrictions = enrich_restrictions_for_injuries(user_profile, restrictions)
    split = choose_split(
    ...
    progression_notes.extend(build_injury_progression_notes(user_profile, restrictions))

    return {
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "generator": "rules_based_v3_low_back_adapted",
        ...
        "safety_disclaimer": (
            "Rutina orientativa generada automáticamente. Si existe dolor intenso, irradiado, progresivo, "
            "hormigueo, pérdida de fuerza o diagnóstico médico, se recomienda consultar a un profesional sanitario."
        ) if has_low_back_issue(user_profile, restrictions) else None,
    }
